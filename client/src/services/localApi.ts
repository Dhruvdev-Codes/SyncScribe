...
const demoTag = (extra: string): string => `\n\n_${extra}_`;
...
export const localAiApi = {
  async generate(
    ...
    if (lower.includes('agenda') || lower.includes('meeting')) {
      text = `# Meeting Agenda\n\n## 1. Objectives\n- Align on goals related to: "${prompt}"\n- Assign owners and deadline for each action item\n\n## 2. Topics to cover\n- ${ctx ? ctx.split(/\s+/).slice(0, 8).join(' ') : 'Current project status'}\n- Risks, blockers, and next steps\n\n## 3. Attendees\n- Add attendees here\n\n## 4. Action items\n- [ ] Owner to confirm deliverables`;
    } else if (lower.includes('blog') || lower.includes('post') || lower.includes('article')) {
      text = `# ${prompt.replace(/generate|write|create|a|an|blog|post|article/gi, '').trim() || 'Blog Post'}\n\n**Introduction**\nHook your reader with a compelling opening about "${prompt}".\n\n**Main points**\n1. Key insight one\n2. Key insight two\n3. A concrete example or case study\n\n**Conclusion**\nSummarise the takeaways and end with a call to action.`;
    } else if (lower.includes('outline') || lower.includes('plan')) {
      text = `## Outline\n1. **Context** — ${prompt}\n2. **Research** — ${ctx ? 'Key facts from your document' : 'Gather supporting material'}\n3. **Structure**\n   - Section A: core argument\n   - Section B: evidence\n   - Section C: counter-arguments\n4. **Next actions** — draft, review, publish`;
    } else {
      text = `Here's a starting point for "${prompt}" (offline demo engine).\n\n- **Definition**: Write one clear sentence that defines the topic.\n- **Why it matters**: Add context — ${ctx ? 'your document mentions: ' + ctx.split(/\s+/).slice(0, 12).join(' ') + '…' : 'connect it to your broader goals.'}\n- **Details**: Bullet out the supporting points.\n- **Next step**: Turn the strongest point into concrete action.`;
    }
    return {
      text:
        text +
        demoTag('Generated by the built-in offline demo engine — connect an AI provider for full generation.'),
    };
  },
  ...
  async rewrite(data: {
    ...
    const intro: Record<string, string> = {
      formal: 'From an executive perspective, the following statement merits attention:',
      casual: "Let's be real here —",
      concise: '',
      creative: 'In vivid strokes of imagination:',
    };
    return {
      text: `${intro[tone] || ''}${intro[tone] ? '\n\n' : ''}${original}${demoTag('Offline demo rewrite — connect an AI provider for full tone transformations.')}`,
    };
  },
  ...
  async chat(
    ...
    const response = `Here's a quick response from the offline demo Copilot.\n\nYou asked: "${last.length > 80 ? last.slice(0, 80) + '…' : last}"\n\n${
      ctx
        ? `I also looked at your document (${ctx.length} characters). Key suggestion: break your content into clear sections with headings, then turn the main idea into a bulleted action list.`
        : 'To get the most from me, open a document so I can read its context alongside your prompts.'
    }\n\nConnect an AI provider (OpenAI/Gemini/Anthropic) in Settings to unlock the full chat assistant.`;
  ...
  async extractActionItems(text: string): Promise<{ actionItems: any }> {
    ...
    return {
      actionItems: items.length
        ? items
        : [{ id: 'ai-none', text: 'No action items detected in this document.', owner: '-', status: 'none' }],
    };
  },

  async generateFaq(text: string): Promise<{ faq: any }> {
    const t = (text || '').trim();
    const snippet = t ? t.slice(0, 120) + '…' : 'This document is empty.';
    return {
      faq: [
        { question: 'What is this document about?', answer: snippet },
        {
          question: 'Who is this document for?',
          answer: t
            ? 'Designed for your intended audience — review the opening sections for explicit context.'
            : 'No content yet — start writing to receive tailored answers.',
        },
        {
          question: 'What should I do next?',
          answer: 'Add a conclusion or action-item section, then share the document with your collaborators.',
        },
      ],
    };
  },
  ...
  async translate(
    ...
    let translated = text || '';
    if (targetLanguage && targetLanguage.toLowerCase().includes('spanish')) {
      const dict: Record<string, string> = {
        ' the ': ' el ', ' a ': ' un ', ' and ': ' y ', ' of ': ' de ', ' to ': ' para ',
        ' is ': ' es ', ' are ': ' son ', ' with ': ' con ', ' in ': ' en ', ' on ': ' en ',
        ' for ': ' para ', ' document ': ' documento ', ' team ': ' equipo ', ' project ': ' proyecto ',
        ' product ': ' producto ', ' meeting ': ' reunión ', ' version ': ' versión ', ' Hello ': ' Hola ',
        ' Welcome ': ' Bienvenido ', ' Summary ': ' Resumen ', ' Today ': ' Hoy ',
      };
      for (const en of Object.keys(dict)) {
        translated = translated.split(en).join(dict[en]);
      }
    }
    return {
      translatedText: `${translated}${demoTag(`Offline demo translation · connect an AI provider for the full ${targetLanguage} engine.`)}`,
      targetLanguage,
    };
  },
};